...
echo
echo "▸ 检查辅助功能权限…"
if ! "$BIN_DIR/miremote" status | grep -q "辅助功能权限: ✅"; then
  cat <<'MSG'
  ❌ 未授权。请执行：
...
echo
echo "▸ 注册开机自启…"
"$BIN_DIR/miremote" install

echo
"$BIN_DIR/miremote" status
echo
echo "✅ 完成。把 $BIN_DIR 加进 PATH 即可直接用 miremote 命令："
